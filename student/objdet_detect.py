...
def create_model(configs):
    ...
    elif 'fpn_resnet' == configs.arch:
        print('using ResNet architecture with feature pyramid')
        
        ####### ID_S3_EX1-4 START #######     
        #######
        print("student task ID_S3_EX1-4")
        model = get_pose_net(num_layers=18, heads=configs.heads, head_conv=configs.head_conv, imagenet_pretrained=configs.imagenet_pretrained)
        #######
        ####### ID_S3_EX1-4 END #######     
...
def detect_objects(input_bev_maps, model, configs):
    ...
    if len(detections)>0:
        ...
        for obj in detections:
            id, bev_y, bev_x, z, h, bev_w, bev_l, yaw = obj
