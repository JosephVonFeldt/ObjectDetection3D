...
def show_pcl(pcl):
    ...
    pointCloud.points = open3d.utility.Vector3dVector(pcl[:, :3])
    # step 4 : for the first frame, add the pcd instance to visualization using add_geometry; for all other frames, use update_geometry instead
    #open3d.visualization.draw_geometries([pointCloud])
    viz.add_geometry(pointCloud)
    # step 5 : visualize point cloud and keep window open until right-arrow is pressed (key-code 262)
    viz.register_key_callback(262, key_callback)
    viz.run()
...
def show_range_image(frame, lidar_name):
    ...
    lidar = [l for l in frame.lasers if l.name ==lidar_name][0]
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    bev_dis = (configs.lim_x[1] - configs.lim_x[0]) / configs.bev_height
    ## step 2 : create a copy of the lidar pcl and transform all metrix x-coordinates into bev-image coordinates
    lidar_pcl_cpy = np.copy(lidar_pcl)
    lidar_pcl_cpy[:, 0] = np.int_(lidar_pcl_cpy[:, 0] / bev_dis)
    ...
    lidar_pcl_top = lidar_pcl_cpy[np.lexsort([-lidar_pcl_cpy[:, 2], lidar_pcl_cpy[:,0 ], lidar_pcl_cpy[:, 1]])]
